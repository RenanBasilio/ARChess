...
public class PickingCube : MonoBehaviour {
	...
	int[] square (GameObject piece) {
		int[] ret = {
			(int) Mathf.Round (Mathf.Abs (piece.transform.position.x - 14) / 4 + 1),
			(int) Mathf.Round ((float) ((piece.transform.position.y - 2.32) / 0.26 + 1))
		};
		return ret;
	}
	...
	void addCubesDiagonally (GameObject piece) { //add conditions in if's
		...
		int[] sqCube;
		GameObject cube;
		if (!(piece.name.Split () [0] == "Black" && square (piece) [0] == 1) && !(piece.name.Split () [0] == "Black" && square (piece) [1] == 8) && !(piece.name.Split () [0] == "White" && square (piece) [0] == 8) && !(piece.name.Split () [0] == "White" && square (piece) [1] == 1))
			while (true) {
				cube = GameObject.CreatePrimitive (PrimitiveType.Cube);
				cube.transform.rotation = selectedPiece.transform.rotation;
				cube.transform.position = selectedPiece.transform.position;
		...
		i = 1;
		if (!(piece.name.Split () [0] == "Black" && square (piece) [0] == 1) && !(piece.name.Split () [0] == "Black" && square (piece) [1] == 1) && !(piece.name.Split () [0] == "White" && square (piece) [0] == 8) && !(piece.name.Split () [0] == "White" && square (piece) [1] == 8))
			while (true) {
				cube = GameObject.CreatePrimitive (PrimitiveType.Cube);
				cube.transform.rotation = selectedPiece.transform.rotation;
				cube.transform.position = selectedPiece.transform.position;
		...
		i = 1;
		if (!(piece.name.Split () [0] == "Black" && square (piece) [0] == 8) && !(piece.name.Split () [0] == "Black" && square (piece) [1] == 8) && !(piece.name.Split () [0] == "White" && square (piece) [0] == 1) && !(piece.name.Split () [0] == "White" && square (piece) [1] == 1))
			while (true) {
				cube = GameObject.CreatePrimitive (PrimitiveType.Cube);
				cube.transform.rotation = selectedPiece.transform.rotation;
				cube.transform.position = selectedPiece.transform.position;
		...
		i = 1;
		if (!(piece.name.Split () [0] == "Black" && square (piece) [0] == 8) && !(piece.name.Split () [0] == "Black" && square (piece) [1] == 1) && !(piece.name.Split () [0] == "White" && square (piece) [0] == 1) && !(piece.name.Split () [0] == "White" && square (piece) [1] == 8))
			while (true) {
				cube = GameObject.CreatePrimitive (PrimitiveType.Cube);
				cube.transform.rotation = selectedPiece.transform.rotation;
				cube.transform.position = selectedPiece.transform.position;
	...
	void addCubesKing (GameObject piece) {
		...
		if (forward && left) {
			...
			cube.transform.rotation = selectedPiece.transform.rotation;
			cube.transform.position = selectedPiece.transform.position;
			cube.transform.Translate (4, -4, 0); //diagonal forward-left
			newpos.Add (cube);
		}
		if (forward && right) {
			cube = GameObject.CreatePrimitive (PrimitiveType.Cube);
			cube.transform.rotation = selectedPiece.transform.rotation;
			cube.transform.position = selectedPiece.transform.position;
			cube.transform.Translate (4, 4, 0); //diagonal forward-right
			newpos.Add (cube);
		}
		if (backward && left) {
			cube = GameObject.CreatePrimitive (PrimitiveType.Cube);
			cube.transform.rotation = selectedPiece.transform.rotation;
			cube.transform.position = selectedPiece.transform.position;
			cube.transform.Translate (-4, -4, 0); //diagonal backward-left
			newpos.Add (cube);
		}
		if (backward && right) {
			cube = GameObject.CreatePrimitive (PrimitiveType.Cube);
			cube.transform.rotation = selectedPiece.transform.rotation;
			cube.transform.position = selectedPiece.transform.position;
			cube.transform.Translate (-4, 4, 0); //diagonal backward-right
			newpos.Add (cube);
		}
	}
